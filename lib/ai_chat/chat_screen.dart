...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({Key? key, required this.userId, required this.apiBase})
      : super(key: key);
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _addWelcomeMessage() {
    _messages.add({
      "sender": "ai",
      "text":
      "👋 Hello! I'm your AI study assistant. I can help you with programming concepts, data structures, algorithms, and more. What would you like to learn today?"
    });
  }
  ...
  Future<void> _loadChatHistory() async {
    try {
      final response = await http.get(
        Uri.parse(
            "${widget.apiBase}/api/chat/history?user_id=${widget.userId}"),
  ...
  /// Load remaining daily messages
  Future<void> _loadUsage() async {
    try {
      final response = await http.get(
        Uri.parse("${widget.apiBase}/api/chat/usage?user_id=${widget.userId}"),
      ).timeout(Duration(seconds: 20));

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        setState(() {
          _remainingMessages = data["remaining_messages"] ?? -1;
        });
      }
    } catch (e) {
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final aiReply = data["reply"] ?? "No reply from AI.";

        setState(() {
          _messages.add({"sender": "ai", "text": aiReply});
          _remainingMessages =
              data["remaining_messages"] ?? _remainingMessages;
        });
      } else {
        setState(() {
          _messages.add({
        ...
        children: [
          if (_remainingMessages >= 0)
            Padding(
              ...
              child: Text(
                "Remaining messages today: $_remainingMessages",
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText:
                      "Ask about programming, algorithms, data structures...",
                      border: OutlineInputBorder(),
                    ),
                    onSubmitted: sendMessage,
                  ),
