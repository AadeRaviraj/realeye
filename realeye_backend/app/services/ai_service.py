...
# --- CREATE INDEX (Run once manually or via a setup script) ---
# This command creates an index to make history queries lightning fast.
# It only needs to be executed ONE TIME.
try:
    chat_collection.create_index(
        columns=[("user_id", "ASC"), ("created_at", "ASC")],
...
def get_chat_history(user_id, limit=30, offset=0):
    """
    Fetch chat history for a specific user with pagination.
    Only returns messages belonging to the given user_id.
    """
    try:
        # Sort by created_at descending to get newest first, then apply limit/offset
        messages = list(chat_collection.find(
            {"user_id": user_id},
...
def call_groq(prompt):
    ...
    data = {
        "model": "llama-3.1-8b-instant",
        "messages": [
            {"role": "system", "content": "You are an interview preparation assistant. Answer clearly with simple explanations and examples."},
